import"./modulepreload-polyfill-B5Qt9EMX.js";import{l as m}from"./utils-B75w2TNQ.js";import{c as u,o as p}from"./auth-Rb6YfhkF.js";m();const s=u();function n(o){const e=document.createElement("div");return e.textContent=String(o),e.innerHTML}s&&g(s);async function g(o){try{const{unauthorized:e,data:t}=await p(o);if(e){window.location.href="/WDD330/login/index.html?message=Session expired. Please login again.";return}const i=document.querySelector("#orders-list");if(!i)return;if(!t||t.length===0){i.innerHTML="<p>No orders found.</p>";return}const c=new Intl.NumberFormat("en-US",{style:"currency",currency:"USD"});i.innerHTML=t.map(r=>{const a=new Date(r.orderDate),l=isNaN(a.getTime())?"N/A":a.toLocaleDateString(),d=c.format(r.orderTotal||0);return`
            <li class="order-card">
                <h3>Order #${n(r.id)}</h3>
                <p>Date: ${n(l)}</p>
                <p>Total: ${n(d)}</p>
